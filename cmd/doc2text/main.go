...
func main() {
	...
	grpcSrv := startGRPCServer(cfg.Server.Addr, bus, logger)
...
func startGRPCServer(addr string, bus *cqrs.Bus, l logger.Logger) *grpc.Server {
	...
	l.Info("gRPC listening on %s", addr)
	go func() {
